...
async function runYtDlp(args) {
  ...
  for (const [command, commandArgs] of attempts) {
    try {
      ...
      if (!/ENOENT|not installed|failed to start/i.test(error.message || '')) {
        throw error;
      }
    }
  }
  ...
  throw new Error(
    'yt-dlp is unavailable in this runtime. Install yt-dlp binary (recommended) or Python + yt_dlp module. ' +
...
export async function downloadCollection(sourceUrl, endImageFile, mode = 'all') {
  ...
  const metadataResult = await runYtDlp(metadataArgs);
  ...
  const selectedEntries = mode === 'one' ? entries.slice(0, 1) : entries;
  const processed = [];
  ...
  for (const item of selectedEntries) {
    ...
    await runYtDlp(['-f', 'mp4', '-o', outputTemplate, item.webpageUrl]);
